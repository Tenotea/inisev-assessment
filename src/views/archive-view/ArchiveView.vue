<template>
  <section class="inbox">
    <div class="inbox__heading">
      <h5 class="inbox__title--primary">Archive</h5>
    ...
    <div class="inbox__actions-container">
      <CheckBox
        ...
        :checked="selectedMessages.length === getArchivedMessage.length"
      />
      <BaseButton
        @click="
    ...
    <ul class="inbox__items-list">
      <InboxListItem
        v-for="message in getArchivedMessage"
        :key="message.id"
        :message="message"
...
export default defineComponent({
  ...
  methods: {
    ...
    selectAllMessage(checked: boolean) {
      if (checked) {
        this.selectedMessages = this.getArchivedMessage.map(
...
@import "../inbox-view/InboxView.scss";
</style>
